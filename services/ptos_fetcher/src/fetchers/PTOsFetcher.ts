...
export default class PTOsFetcher {
  ...
  async fetch(): Promise<Array<PTO>> {
    ...
    const blockIds = ptosContent.data.result.reducerResults.calendar_results.blockIds;
    const ptos: PTO[] = [];
    blockIds.forEach((blockId: string) => {
      const blockProps = ptosContent.data.recordMap.block[blockId].value.properties;
  ...
  private async queryCollection(): Promise<AxiosResponse> {
    ...
    return await axios("https://www.notion.so/api/v3/queryCollection", {
      method: 'POST',
